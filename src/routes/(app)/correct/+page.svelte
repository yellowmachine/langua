	...
	{:else if result !== 'idle' && result !== 'loading'}
		{#if result.corrections.length === 0}
			<p class="text-sm" style:color="var(--color-ink-muted)">
				{result.overallFeedback ?? 'No se han encontrado errores.'}
			</p>
		{:else}
			<ul class="flex flex-col gap-2">
				{#each result.corrections as item, i (i)}
					<li
			...
			{#if result.overallFeedback}
				<p class="text-sm" style:color="var(--color-ink-muted)">{result.overallFeedback}</p>
			{/if}
		{/if}
	{/if}
</div>
